...
export class MedicalConsultationService implements IMedicalConsultationService {
  ...
  CreateMedicalConsultation(medicalConsultation: MedicalConsultationDTO): Observable<ResponseDTO> {
    return this._configService.getUrl().pipe(
      switchMap(url => {
        return this._httpService.post(url, "CreateMedicalConsultation", null, medicalConsultation);
      })
    );
  }
  ...
  UpdateMedicalConsultation(medicalConsultation: MedicalConsultationDTO): Observable<ResponseDTO> {
    return this._configService.getUrl().pipe(
      switchMap(url => {
        return this._httpService.put(url, "UpdateMedicalConsultation", null, medicalConsultation);
      })
    );
  }
  ...
  CreateMedicalConsultationWithMedicalDiagnosis(medicalDiagnosis: MedicalConsultationDTO): Observable<ResponseDTO> {
    return this._configService.getUrl().pipe(
      switchMap(url => {
        return this._httpService.post(url, "CreateMedicalConsultationWithMedicalDiagnosis", null, medicalDiagnosis);
      })
    );
  }
